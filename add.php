...
if (isset($_GET['ajax'])) {
    ...
    if ($ajax === 'provinces') {
        ...
        $s = $rid
            ? $conn->prepare("SELECT province_id, province_name, region_id FROM table_province WHERE region_id = ? ORDER BY province_name") && $s->execute([$rid])
            ...
            <div class="p-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="payment_terms" class="w-full border border-gray-300 rounded px-3 py-1.5 text-sm focus:border-blue-400 focus:ring-1 focus:ring-blue-300" value="<?= htmlspecialchars($data['payment_terms']) ?>">
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="remarks" rows="2" class="w-full border border-gray-300 rounded px-3 py-1.5 text-sm focus:border-blue-400 focus:ring-1 focus:ring-blue-300"><?= htmlspecialchars($data['remarks']) ?></textarea>
                    </div>

                    <div class="sm:col-span-2">
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Special Instruction</label>
                        <textarea name="special_instruction" rows="2" class="w-full border border-gray-300 rounded px-3 py-1.5 text-sm focus:border-blue-400 focus:ring-1 focus:ring-blue-300"><?= htmlspecialchars($data['special_instruction']) ?></textarea>
                    </div>
                    ...
                    <table class="w-full text-sm border border-gray-200 rounded" id="items-table">
                        ...
                        <tbody id="items-body" class="divide-y divide-gray-100">
                            ...
                            <tr class="item-row">
                                <td class="px-2 py-1.5" style="min-width:220px">
                                    <select name="items[<?= $idx ?>][inventory_id]" class="inv-select w-full border border-gray-300 rounded px-2 py-1 text-sm bg-white focus:border-blue-400" required>
                                ...
                                <td class="px-2 py-1.5"><input type="text" name="items[<?= $idx ?>][item_code]" class="item-code border border-gray-300 rounded px-2 py-1 text-sm w-24 focus:border-blue-400" value="<?= htmlspecialchars($item['item_code'] ?? '') ?>"></td>
                                <td class="px-2 py-1.5"><input type="text" name="items[<?= $idx ?>][item_description]" class="item-desc border border-gray-300 rounded px-2 py-1 text-sm w-36 focus:border-blue-400" value="<?= htmlspecialchars($item['item_description'] ?? '') ?>"></td>
                                <td class="px-2 py-1.5">
                                    <select name="items[<?= $idx ?>][uom]" class="item-uom border border-gray-300 rounded px-2 py-1 text-sm bg-white focus:border-blue-400 outline-none w-24">
                                        <option value="">--</option>
                                ...
                                <td class="px-2 py-1.5"><input type="number" name="items[<?= $idx ?>][quantity]" class="item-qty border border-gray-300 rounded px-2 py-1 text-sm w-20 focus:border-blue-400" min="0.0001" step="0.0001" value="<?= htmlspecialchars($item['quantity'] ?? 1) ?>" required></td>
                                <td class="px-2 py-1.5"><input type="number" name="items[<?= $idx ?>][unit_price]" class="item-price border border-gray-300 rounded px-2 py-1 text-sm w-24 focus:border-blue-400" min="0" step="0.01" value="<?= htmlspecialchars($item['unit_price'] ?? 0) ?>" required></td>
                        ...
                        <tfoot>
                            <tr class="bg-gray-50 border-t border-gray-200">
                                ...
                                <td colspan="2" class="px-3 py-2.5"><span id="grand-total" class="font-bold text-blue-600 text-sm">₱0.00</span></td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
